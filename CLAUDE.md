...
### 상수 분리 지침
모든 매직 넘버와 하드코딩된 값은 상수로 분리하여 유지보수성을 향상시킵니다.

#### 상수 분리 대상
- **매직 넘버**: 코드에서 직접 사용되는 숫자 값 (예: 1, 2000, 10 등)
- **반복되는 문자열**: 에러 메시지, 로그 메시지, 설정 값 등
- **설정 값**: 타임아웃, 크기 제한, 개수 제한 등
- **상태 코드**: 비즈니스 로직에서 사용되는 고유한 상태 코드

#### 상수 분리 제외 대상
- **표준 HTTP 상태 코드**: 200, 404, 500 등 (웹 표준이므로 분리 불필요)
- **표준 HTTP 헤더**: Content-Type, Cache-Control 등 (웹 표준이므로 분리 불필요)
- **단순 증감값**: 배열 인덱스 증감, 반복문 카운터 등
...
#### 테스트 작성 제외 대상
다음과 같은 경우 테스트 작성을 하지 않음:
- **상수 파일** (`.constants.ts`): 단순 값 선언만 있는 파일은 테스트 불필요
- **타입 정의 파일** (`.types.ts`): TypeScript 타입만 정의한 파일
- **설정 파일**: 단순 설정값만 export하는 파일
- **인터페이스만 정의한 파일**: 구현체 없이 인터페이스만 있는 파일

#### 테스트 작성 필수 대상
- **서비스 클래스**: 비즈니스 로직이 포함된 클래스
- **유틸리티 함수**: 복잡한 로직이 있는 함수들
- **미들웨어**: 요청/응답 처리 로직
- **외부 API 연동**: 실제 로직이 구현된 클라이언트
...
- **문서**: 한글로 작성
- **⏰ 시간 표기 규칙 (중요)**: **모든 시간 기록은 반드시 한국 시간(KST, UTC+9) 기준으로 작성**
  - 적용 범위: 파일명, MD 문서 내부, 커밋 메시지, 로그 등 모든 시간 기록
  - ❌ 잘못된 예: `2025-01-08`, `250108-1200` (실제 한국 현재 시간과 다른 날짜)
  - ✅ 올바른 예: `2025-08-08`, `250808-1200` (실제 한국 현재 시간 기준)
  - **파일명 시간 형식**: `{YYMMDD}-{HHMM}` (예: `250808-1200`)
  - **문서 내 시간 형식**: `{YYYY-MM-DD HH:MM}` (예: `2025-08-08 12:00`)
  - **작성 전 필수 확인**: 반드시 실제 한국 현재 시간을 확인하고 정확히 기록할 것

## 코드 설명 문서 규칙
- **위치**: `docs/code-explanations/` 폴더
- **파일명 형식**: `{YYMMDD}-{HHMM}-{주제}.md`
  - 예: `20250807-1040-server-implementation.md`
  - **시간 기준**: 한국 시간(KST) 기준으로 작성 (UTC+9)
