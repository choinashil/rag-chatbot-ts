...
**0. 개발 계획 재검토 및 시간 추적 시작**
- 현재 코드 상태를 바탕으로 개발 계획의 적절성 검토
- 이전 단계 구현 과정에서 변경된 사항 반영
- 문제 없으면 바로 진행, 변경 필요 시 사용자 승인 후 진행
- **단계별 실제 소요 시간 추적**: 
  - **사용자 명시적 지시**: 사용자가 "x 단계 개발 시작" 또는 "종료" 메시지를 주면 해당 시간 기준으로 계산
  - **자동 추적 (기본값)**: 사용자 별도 코멘트 없는 경우, 다음 작업 시작 시점에 이전 작업 종료로 간주하여 소요 시간 계산
  - 계산된 소요 시간을 개발 계획 문서에 업데이트
...
- 다음 단계를 위한 인사이트 문서화
- **단계별 실제 소요 시간 기록**: 
  - 사용자 명시적 지시가 있었던 경우: 해당 시간 기준으로 계산
  - 자동 추적인 경우: 0단계 시작 시각과 현재 시각의 차이로 계산
  - 계산된 실제 소요 시간을 개발 계획 문서에 추가
