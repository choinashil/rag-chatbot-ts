...
- [x] **형제 페이지 스킵**: `includeParentPages: false`일 때 상대깊이 ≤ 0인 페이지 모두 스킵 (부모 + 형제)
- [x] **부모/형제 페이지 하위 링크 차단**: 스킵된 페이지의 하위 링크가 큐에 추가되지 않도록 개선
...
```typescript
// 이전: 부모 페이지만 스킵
if (relativeDepth < 0 && !session.options.includeParentPages)

// 현재: 부모 + 형제 페이지 스킵 (시작 페이지 제외)
...
- **형제 페이지**: 상대깊이 0, 탐색깊이 > 0 → ❌ 스킵, 하위 링크 차단
...
**현재 상태**: ✅ Stage 4 완료 - HTML 파서 전략 패턴 리팩토링 완료  
**다음 작업**: Stage 5 - 벡터 데이터베이스 연동  
